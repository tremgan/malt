...
from malt.active_learning.campaign import LabJournal, run_campaign
...
# Each rule, through the real campaign


def campaign(seed_data, rule, *, model=None, random_seed=0, n=1):
...
def test_stopped_by_names_the_rule_that_ended_the_campaign(seed_data):
    rule = UnreliableFitRule() | MaxRoundsRule(4)
    capped = campaign(seed_data, rule, n=3)
    failed = campaign(seed_data, rule, model=CountingModel(fail_at=9), n=3)
...
def test_stopped_by_reports_simultaneous_rules(seed_data):
    ...
    journal = campaign(seed_data, UnreliableFitRule() | MaxRoundsRule(3),
...
def test_unreliable_seed_fit_stops_before_proposing(seed_data):
    journal = campaign(seed_data, UnreliableFitRule() | MaxRoundsRule(10),
...
def test_random_stop_is_reproducible_and_pure(seed_data):
    ...
    a = campaign(seed_data, rule, random_seed=7)
    b = campaign(seed_data, rule, random_seed=7)
    assert len(a.rounds) == len(b.rounds)
    # Re-evaluating on the finished journal agrees with what stopped the campaign.
...
def test_random_stop_extremes(seed_data, p, rounds):
    assert len(campaign(seed_data, RandomStopRule(p) | MaxRoundsRule(5)).rounds) == rounds
